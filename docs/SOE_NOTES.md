...
`h2o.v1.*`, so it is safe to run on a laptop with a student's real work on it. It reports
...
- The vault code was identical to the one the same ID produces locally, which is the
  design: it is a function of their `studentId`, not of where the site is served from.
...
engagement heuristics, so the same laptop can answer false today and true after the student
...
which they do. Worth re-reading on the SOE, because a managed profile may differ again.
...
that should cost a student the door.
...
their controls in a `finally`. Worth knowing this failure mode exists if a student reports
